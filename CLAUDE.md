...
- **Course content** is plain TypeScript data: `src/content/course.ts` is the map, one file per lesson under `src/content/lessons/`. The syllabus is `docs/curriculum-a1.md` and `docs/curriculum-a2.md`; units carry their `level`, the map numbers units within a level, and the A1 achievement counts only A1 lessons.
